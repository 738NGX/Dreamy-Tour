...
import { RabbitRetry } from "@/decorator/rabbitRetry";
...
class EmailConsumer extends RabbitMQConsumer {
  ...
  protected async handleMessage(channel: Channel, msg: ConsumeMessage | null): Promise<void> {
    // 装饰器已经处理过消息为空的情况，这里直接将消息声明为 ConsumeMessage
    const emailCodeDto = JSON.parse((msg as ConsumeMessage).content.toString()) as EmailCodeDto;
    let reminder = ""
